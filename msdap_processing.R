...
observeEvent(input$submit_msdap, {
    ...
    #test contrasts to yaml
    # contrasts_list <- list(dataset$contrasts[[1]]$label_contrast, 
    #                        dataset$contrasts[[2]]$label_contrast)
    # list_of_vectors <- lapply(contrasts_list, function(x) strsplit(x, "_vs_")[[1]])
    # contrast_yaml <- yaml::as.yaml(contrasts_list)
    # writeLines(contrast_yaml, './test.yaml')
