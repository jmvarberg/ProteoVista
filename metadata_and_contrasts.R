...
output$mdsap_run_params <- renderUI({
    ...
    tagList(
        ...
        bsTooltip("msdap_min_pep_per_prot", "This sets the minimum number of unique peptides a protein must have to be retained for downstream analysis.", "bottom", options = list(container = "body")),
        # checkboxInput(
        ...
        checkboxInput(
            ...
            label = "Are there technical replicates that need to be combined? Must include information in 'merge' column of uploaded metadata file",
            value = FALSE,
            width = "100%"
        ),
        bsTooltip("msdap_tech_reps", "If selected, precursors from each replicate are filtered for FDR, then averaged across replicates to derive a single precursor intensity value.", "bottom", options = list(container = "body")),
        selectInput(
        ...
        bsTooltip("dea_qval_thresh", "This selection only influences the hits that are classified as significant in MS-DAP PDF report and SIMR Excell ProteoVista Summary Report. All values are returned in DE results exported as CSV for Quickomics.", "bottom", options = list(container = "body")),
        numericInput(
        ...
        bsTooltip("dea_log2sig_thresh", "This selection only influences the hits that are classified as significant in MS-DAP PDF report and SIMR Excell ProteoVista Summary Report. All values are returned in DE results exported as CSV for Quickomics. Defalut value returns all hits based solely on FDR threshold (no logFC requirements).", "bottom", options = list(container = "body")),
        checkboxInput(
            inputId = "msdap_do_dd",
...
output$mdsap_dd_filt_params <- renderUI({
    ...
    tagList(
        ...
        bsTooltip("msdap_min_pept_dd", "", "bottom", options = list(container = "body")),
        numericInput(
        ...
        bsTooltip("msdap_min_samples_dd", "", "bottom", options = list(container = "body"))
